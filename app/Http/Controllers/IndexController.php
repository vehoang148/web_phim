...
class IndexController extends Controller
{
    public function loc_phim(){
        ...
        if($order=='' && $genre_filter=='' && $country_filter=='' && $year_filter==''){
            ...
            $meta_image = '';
        $movie_array = Movie::withCount('episode');//đếm từng phim
        if($country_filter){
            $movie_array = $movie_array->where('country_id',$country_filter);
        }
        if($year_filter){
            $movie_array = $movie_array->where('year','=',$year_filter);
        }
        if($order){
            $movie_array = $movie_array->where('year','=',$order);
        }
        elseif($order){
        ...
        foreach($movie_array as $mov){
            foreach($mov->movie_genre as $mov_gen){
                $movie = $movie_array->whereIn('genre_id',[$mov_gen]);
            }
    ...
    public function  home()
    {
        ...
        return view('pages.home', compact('phim_hot', 'category', 'genre', 'country', 'category_home','phimhot_sidebar'));
    }
    ...
    public function movie($slug)
{
    ...
    $movie = Movie::with('category', 'genre', 'country','movie_genre')->where('slug', $slug)->where('status', 1)->first();
        ...
        return view('pages.movie', compact('category', 'genre', 'country', 'movie', 'related', 'tags', 'phimhot_sidebar', 'episode_3_tapmoi','episode_tapdau','episode_all'));
